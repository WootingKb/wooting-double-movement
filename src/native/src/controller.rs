...
impl JoystickState {
    ...
    pub fn get_basic_direction(&self, config: Option<&ServiceConfiguration>) -> (f32, f32) {
        ...
        let mut angle: f32 = config.map(|v| v.left_joystick_angles.right_angle).unwrap_or(0.67);
        ...
        if is_advanced_strafe_on && left && !up && !down && !right {
            y = 1.0;
            x = -1.0;
            angle = config.map(|v| v.left_strafe_joystick_angles.right_angle).unwrap_or(0.67);
        } 
